...
{
	if(controller_map[Controller::current_controller] == ANALOG_CONTROLLER){
		// Store values in ADC register (MAke sure to read ADCL first, to enable change in ADCH. See page 219 in datasheet)
		Controller::controller_vals[Controller::current_controller] = ADCL | (ADCH << 8);
	}
	else if(controller_map[Controller::current_controller] == DIGITAL_CONTROLLER){
		Controller::controller_vals[Controller::current_controller] = (PINC & (1 << Controller::input_map[Controller::current_controller])) ? 1 : 0;
	}
}
